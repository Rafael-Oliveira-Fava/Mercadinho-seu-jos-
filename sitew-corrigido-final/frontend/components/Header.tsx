...
const Header: React.FC<HeaderProps> = ({ activeSection, setSection, cartCount, toggleCart, toggleMenu, user, onLogout }) => {
  ...
  return (
    <>
      {/* Header Desktop/Mobile Superior */}
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-jose-dark/10 shadow-2xl pb-safe">
        <div className="grid grid-cols-5 h-20">
          <button
            onClick={() => setSection(AppSection.Home)}
            className={`flex flex-col items-center justify-center gap-1 transition-all ${
              activeSection === AppSection.Home ? 'text-jose-primary' : 'text-jose-dark/40'
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center gap-1 transition-all ${
              activeSection === AppSection.Products ? 'text-jose-primary' : 'text-jose-dark/40'
            }`}
          >
            <i className="fas fa-leaf text-xl"></i>
            <span className="text-[9px] font-bold uppercase tracking-wider">Horta</span>
          </button>

          <button
            onClick={() => setSection(AppSection.SmartChef)}
            className={`flex flex-col items-center justify-center gap-1 transition-all ${
              activeSection === AppSection.SmartChef ? 'text-jose-primary' : 'text-jose-dark/40'
            }`}
          >
            <i className="fas fa-robot text-xl"></i>
            <span className="text-[9px] font-bold uppercase tracking-wider">Chef</span>
          </button>

          <button
            onClick={() => setSection(AppSection.Location)}
            className={`flex flex-col items-center justify-center gap-1 transition-all ${
              activeSection === AppSection.Location ? 'text-jose-primary' : 'text-jose-dark/40'
            }`}
          >
          ...
          {user ? (
            <button
              ...
              className={`flex flex-col items-center justify-center gap-1 transition-all relative ${
                userMenuOpen ? 'text-jose-primary' : 'text-jose-dark/40'
              }`}
            >
              <div className="w-6 h-6 bg-jose-dark rounded-full flex items-center justify-center text-white text-xs font-bold">
                {user.name.charAt(0).toUpperCase()}
              </div>
              <span className="text-[9px] font-bold uppercase tracking-wider">Perfil</span>
              
              {userMenuOpen && (
                <div className="absolute bottom-full right-0 mb-2 w-64 bg-white rounded-2xl shadow-2xl border border-jose-dark/5 overflow-hidden">
                  <div className="p-6 bg-jose-light border-b border-jose-dark/5">
                    <p className="text-sm font-bold text-jose-dark">{user.name}</p>
                    <p className="text-xs text-gray-500 mt-1">{user.email}</p>
                    {user.role === 'owner' && (
            ...
            <button
              onClick={() => setSection(AppSection.Login)}
              className={`flex flex-col items-center justify-center gap-1 transition-all ${
                activeSection === AppSection.Login ? 'text-jose-primary' : 'text-jose-dark/40'
              }`}
            >
